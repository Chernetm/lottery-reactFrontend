    ...
    return (
        <div className="bg-slate-50 min-h-screen">
            <div className="max-w-7xl mx-auto px-6 py-12">
                <div className="mb-12">
                    <h1 className="text-4xl font-bold mb-4 text-slate-900">Available Lotteries</h1>
                    <p className="text-slate-600">Choose your lucky draw and win high-end tech, gadgets, and more.</p>
                </div>
                ...
                <div className="flex flex-col md:flex-row gap-4 mb-12">
                    <div className="relative flex-grow">
                        ...
                        <input 
                            ...
                            className="w-full bg-white border border-slate-200 rounded-xl py-3 pl-12 pr-4 focus:border-brand-primary outline-none transition text-slate-900 shadow-sm"
                        />
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {filteredLotteries.map((lottery) => (
                            <motion.div
                                ...
                                whileHover={{ y: -10 }}
                                className="bg-white rounded-3xl overflow-hidden group border border-slate-200 hover:border-brand-primary/30 transition-all duration-300 shadow-sm hover:shadow-xl"
                            >
                                <div className="h-56 overflow-hidden relative">
                                    ...
                                    <div className="absolute top-4 right-4 flex flex-col gap-2 items-end">
                                        ...
                                        {lottery.prizes && lottery.prizes.length > 0 && (
                                            <div className="bg-white/90 backdrop-blur-sm border border-brand-primary/30 px-3 py-1 rounded-full text-[10px] font-bold text-brand-primary shadow-lg flex items-center gap-1">
                                ...
                                <div className="p-8">
                                    <h3 className="text-2xl font-bold mb-2 text-slate-900 group-hover:text-brand-primary transition-colors">{lottery.item.name}</h3>
                                    <p className="text-slate-600 text-sm mb-8 line-clamp-2">{lottery.item.description}</p>
                                    ...
                                    <div className="flex justify-between items-center mb-8 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                        ...
                                        <div className="text-right">
                                            ...
                                            <p className="text-lg font-bold text-slate-900">{lottery.totalTickets}{lottery.maxTickets ? ` / ${lottery.maxTickets}` : ''}</p>
                                        </div>
                                    </div>
                    ...
                    <div className="bg-white p-20 rounded-3xl text-center border border-dashed border-slate-200">
                        <p className="text-slate-600 text-lg">No lotteries found matching your search.</p>
                    </div>
                )}
            </div>
        </div>
