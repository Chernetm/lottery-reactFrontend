    ...
    return (
        <div className="max-w-7xl mx-auto px-6 py-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                ...
                >
                    <div className="rounded-3xl overflow-hidden glass-effect p-2 mb-8 relative">
                        <img
                        ...
                        {lottery.status === 'DRAWN' && (
                            <div className="absolute top-6 right-6 px-6 py-2 bg-brand-primary rounded-full font-bold shadow-2xl flex items-center gap-2 animate-pulse">
                    ...
                    <div className="space-y-6">
                        <div className="flex justify-between items-center">
                            <h1 className="text-4xl font-extrabold">{lottery.item.name}</h1>
                            <div className={`p-4 rounded-2xl glass-effect ${lottery.status === 'DRAWN' ? 'text-brand-primary' : 'text-brand-secondary'}`}>
                        ...
                        <p className="text-slate-400 text-lg leading-relaxed">{lottery.item.description}</p>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                {lottery.prizes?.map((prize, idx) => (
                                    <div key={idx} className="glass-effect p-4 rounded-2xl flex items-center gap-4 border border-slate-700/50">
                                ...
                                {(!lottery.prizes || lottery.prizes.length === 0) && (
                                    <div className="glass-effect p-4 rounded-2xl flex items-center gap-4">
                                        ...
                                        <p className="font-bold">{lottery.item.name}</p>
                                    </div>
                                )}
                            </div>
                        </div>
                ...
                <motion.div
                    ...
                    className="lg:pt-4"
                >
                    <div className="glass-effect p-8 md:p-10 rounded-3xl sticky top-28 border border-brand-primary/20">
                        {lottery.status === 'DRAWN' ? (
                            <div className="space-y-8 text-center py-4">
                                ...
                                <div>
                                    <h3 className="text-3xl font-black mb-2">We Have A Winner!</h3>
                                    <p className="text-slate-500">The provably fair draw has been completed</p>
                                </div>

                                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                    {lottery.prizes?.map((prize, idx) => (
                                        <div key={idx} className="bg-slate-800/50 p-6 rounded-3xl border border-brand-primary/30 relative overflow-hidden group text-left">
                                    ...
                                    {(!lottery.prizes || lottery.prizes.length === 0) && (
                                        <div className="bg-slate-800/50 p-8 rounded-3xl border border-brand-primary/30 relative overflow-hidden group">
                                    ...
                                    <div className="mt-4 flex items-center justify-center gap-2 text-sm text-green-400 font-bold">
                                        <ShieldCheck size={16} /> Verified Provably Fair
                                    </div>
                                </div>

                                <div className="space-y-4 pt-6">
                                    <p className="text-slate-400 text-sm">Better luck next time if you didn't win! Check out other active lotteries.</p>
                                    <Link to="/" className="block w-full premium-gradient py-5 rounded-2xl font-bold text-lg text-white transition-all shadow-xl shadow-brand-primary/20">
                                        View All Lotteries
                                    </Link>
                                </div>
                            </div>
                                ...
                                <AnimatePresence mode="wait">
                                    {success ? (
                                        ...
                                        >
                                            ...
                                            <h4 className="text-xl font-bold text-green-400">Tickets Purchased!</h4>
                                            <p className="text-slate-400 text-sm">Your order was successful. Redirecting to your dashboard...</p>
                                        </motion.div>
                                    ) : (
                                        <div className="space-y-8">
                                            <div className="space-y-4">
                                                <label className="text-sm font-bold text-slate-300 ml-1">Select Quantity</label>
                                                <div className="flex items-center gap-4">
                                                    <button
                                                        onClick={() => setQuantity(Math.max(1, quantity - 1))}
                                                        className="w-14 h-14 rounded-xl bg-slate-800 border border-slate-700 hover:border-brand-primary flex items-center justify-center text-2xl font-bold transition"
                                                    >
                                                        -
                                                    </button>
                                                    <div className="flex-grow bg-slate-800/50 border border-slate-700 rounded-xl h-14 flex items-center justify-center text-xl font-bold font-mono">
                                                        {quantity}
                                                    </div>
                                                    <button
                                            ...
                                            <div className="pt-6 border-t border-slate-800">
                                                <div className="flex justify-between items-center mb-8">
                                                    <p className="text-lg font-medium text-slate-400">Total Price</p>
                                                    <p className="text-3xl font-black">${totalPrice}</p>
                                                </div>

                                                {error && (
                                                    <div className="bg-red-500/10 border border-red-500/50 text-red-500 p-4 rounded-xl mb-6 flex items-start gap-2 text-sm">
