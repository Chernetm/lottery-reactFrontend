            ...
            {statCards.map((stat, i) => (
                stat.link ? (
                    <Link
                        key={i}
                        to={stat.link}
                        className="glass-effect p-5 md:p-8 rounded-2xl md:rounded-3xl flex items-center justify-between hover:border-brand-primary/50 hover:-translate-y-1 transition-all group border border-slate-700"
                    >
                        <div>
                            <p className="text-slate-500 text-[10px] md:text-sm font-semibold mb-0.5 md:mb-1 uppercase tracking-wider">{stat.label}</p>
                            <h3 className="text-xl md:text-3xl font-extrabold text-white group-hover:text-brand-primary transition">{stat.value}</h3>
                        </div>
                    ...
                    <motion.div
                        ...
                        className={`glass-effect p-5 md:p-8 rounded-2xl md:rounded-3xl flex items-center justify-between hover:-translate-y-1 transition-all border border-slate-700 ${stat.label === 'Winning Draws' ? 'cursor-pointer hover:border-brand-accent/50' : ''}`}
                    >
                        <div>
                            <p className="text-slate-500 text-[10px] md:text-sm font-semibold mb-0.5 md:mb-1 uppercase tracking-wider">{stat.label}</p>
                            <h3 className="text-xl md:text-3xl font-extrabold text-white">{stat.value}</h3>
